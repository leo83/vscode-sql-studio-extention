...
**Результаты запроса (таблица)** — сортировка, фильтр, пагинация, экспорт:

![Результаты запроса — таблица](docs/images/results-table.png)

**Результаты запроса (график)** — pie, bar, scatter, heatmap по колонкам результата. Для pie с большим числом категорий — прокручиваемая легенда; pinch-to-zoom (жест двумя пальцами или Ctrl/Cmd + колёсико) на области графика и скролл тачпадом по легенде:

![Результаты запроса — график](docs/images/results-chart.png)
