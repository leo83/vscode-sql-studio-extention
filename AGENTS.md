...
Расширение использует **[Semantic Versioning](https://semver.org/)** (`MAJOR.MINOR.PATCH`).

### Когда что менять

| Уровень | Когда поднимать | Примеры |
|---------|----------------|---------|
| **PATCH** `0.1.x` | Баг-фикс без новых фич; правка документации | Починить краш экспорта XLSX, исправить NULL в результатах, fix опечатки |
| **MINOR** `0.x.0` | Новая пользовательская функциональность; обратно совместимо | Refresh-кнопка, новый диалект БД, новый webview, новый тип экспорта |
| **MAJOR** `x.0.0` | Ломающее изменение: смена формата данных, протокола, удаление команд | Смена JSON-RPC протокола, переименование всех команд, drop Python 3.11 |

**Правило для pre-1.0:** пока версия `0.x.y`, MINOR используется как расширенный PATCH — ломающие изменения допустимы в MINOR без поднятия MAJOR. Ориентируйся на:
- `0.x.0` — видимый пользователю сдвиг: набор фич, новый диалект, редизайн UI
- `0.0.x` / PATCH — незаметный для пользователя фикс

### Что менять при инкременте

Обязательно в одном коммите/PR:
...
1. **`package.json`** — поле `"version"`:
   ...
   # npm version создаёт git-тег автоматически; если не нужен — правь вручную
   ```

2. **`CHANGELOG.md`** — переименовать `[Unreleased]` в `[X.Y.Z] — YYYY-MM-DD` и добавить пустой `[Unreleased]` сверху:
   ```markdown
   ## [Unreleased]
...
### Как писать записи в CHANGELOG

Формат: **Keep a Changelog** (`Added` / `Fixed` / `Changed` / `Removed` / `Deprecated` / `Security`).

- Пиши с точки зрения **пользователя**, не разработчика.
- Начинай с жирного названия фичи/команды: `**Refresh button** — ...`
- Баг-фиксы: описывай симптом, не реализацию: `Fix XLSX export crash on array column values` вместо `Add _cell_value() helper in excel_export.py`.
- Не дублируй то, что уже в коммит-сообщении: CHANGELOG — это release notes, не git log.
- Одна запись на видимое изменение, не на каждый коммит.

### Чеклист перед инкрементом версии

- [ ] Все фичи/фиксы этой волны попали в `[Unreleased]` в CHANGELOG
- [ ] `package.json` version обновлён
