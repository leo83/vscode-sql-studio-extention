...
|---------|----------------|---------|
| **PATCH** `0.1.x` | Баг-фиксы; мелкие улучшения UX (иконки, кнопка Refresh, tooltip); правка документации | Краш XLSX на массивах, NULL в dotted-колонках, иконки на кнопках, кнопка Refresh |
| **MINOR** `0.x.0` | Принципиально новая функциональность для пользователя | Новый диалект БД, новый тип webview, новый способ экспорта, ER-диаграмма |
...
**Эмпирическое правило:** если изменение заметно пользователю, но не даёт ему новый рабочий процесс — это PATCH. Если пользователь получает новый способ решить задачу — MINOR.
